...
#SBATCH --job-name=paml_M0_2R_bsA # 中文注释：作业名称，修改为 M0, 2-ratio, Branch-site A
#SBATCH --output=paml_M0_2R_bsA_%j.out # 中文注释：标准输出文件
#SBATCH --error=paml_M0_2R_bsA_%j.err  # 中文注释：标准错误文件
...
# 修改 TREE_DIR_PATH 指向新的包含 M0 提取并标记的树的目录
TREE_DIR_PATH="$PROJECT_BASE_PATH/gene_trees_from_M0_remarked"
# 修改 OUTPUT_DIR_NAME 以反映其内容
OUTPUT_DIR_NAME="$PROJECT_BASE_PATH/paml_M0_2R_bsA_on_M0trees"
...
BSA_ALT_CTL_TEMPLATE="$PROJECT_BASE_PATH/bsA_alt.ctl"         # 分支位点模型A (alternative) CTL模板
...
echo "  分支位点模型A CTL模板: $BSA_ALT_CTL_TEMPLATE"
...
if [ ! -f "$BSA_ALT_CTL_TEMPLATE" ]; then
  echo "错误: 分支位点模型A CTL模板 '$BSA_ALT_CTL_TEMPLATE' 未找到。"
  exit 1
fi
...
TREEFILE_PLACEHOLDER_IN_CTL="/home/kosukesano/tools/for_paml/IQTREE_6sp/data/new_tree_IQTREE_ultrametric.nwk" # 确保这个占位符与您的CTL文件实际使用的匹配

# 定义M0模型特定的占位符
M0_OUTFILE_PLACEHOLDER="<geneX_AE_branch_M0_results.txt>" # 确保这个占位符与您的 M0 CTL文件实际使用的匹配

# 定义2-ratio模型特定的占位符
TWO_RATIO_OUTFILE_PLACEHOLDER="<geneX_AE_branch_2ratio_results.txt>" # 确保这个占位符与您的 2-ratio CTL文件实际使用的匹配

# 定义分支位点模型A特定的占位符
BSA_ALT_OUTFILE_PLACEHOLDER="<geneX_AE_bsA_alt_results.txt>" # 确保这个占位符与您的 bsA_alt.ctl 文件实际使用的匹配
...
for seq_file_path in "$SEQ_ALN_DIR"/*_codon.clipkit.fasta; do
  if [ -f "$seq_file_path" ]; then
    ...
    # 修改 current_tree_file 的构建以匹配新的树文件名
    ...
    m0_paml_outfile_path_abs="$ABS_OUTPUT_DIR/${gene_name}_M0_paml_results.txt" # 输出文件名保持简洁
    ...
    current_m0_ctl_content=$(echo "$m0_ctl_template_content" | \
        ...
        sed "s|$M0_OUTFILE_PLACEHOLDER|$m0_paml_outfile_path_abs|g") # 使用 # 作为sed的分隔符，因为路径中可能包含 /
    ...
    echo "正在为 $gene_name 启动PAML (2-ratio 模型)... 工作目录: $two_ratio_workdir ; 树文件: $abs_tree_file_path"
    ...
    # --- 分支位点模型 A (Branch-site Model A - Alternative) ---
    ...
    echo "正在为 $gene_name 启动PAML (分支位点模型 A)... 工作目录: $bsa_alt_workdir ; 树文件: $abs_tree_file_path"
    ...
    echo "已为 $gene_name 提交PAML任务 (M0, 2-ratio, Branch-site A)。CTL文件在各自的 _work 目录中。"
  else
...
wait # 等待所有后台任务执行完毕
echo "所有PAML (M0, 2-ratio, Branch-site A) 分析已完成。结果保存在: $ABS_OUTPUT_DIR (主要结果和日志), 各自的 _work 子目录包含辅助文件。" 